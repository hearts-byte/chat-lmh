...
function applyTopBarPermissions(rank) {
    const canPrivate = !!RANK_PERMISSIONS[rank]?.canSeePrivateChatButton;
    const canReport = !!RANK_PERMISSIONS[rank]?.canSeeReportButton;
    document.documentElement.dataset.canPrivate = canPrivate ? '1' : '0';
    document.documentElement.dataset.canReport = canReport ? '1' : '0';
    try {
        localStorage.setItem('tbPerms', JSON.stringify({ rank, canPrivate, canReport }));
    } catch (e) { /* تجاهل، التخزين تحسين اختياري فقط */ }
}
...
// ... (بقية الكود لا يتغير) ..
    try {
        ...
        applyTopBarPermissions(currentUserRank);
...
profileButton = document.getElementById('profileButton');
        ...
        if (currentUserId) {
            try {
                ...
                const currentUserData = allUsersAndVisitors.find(user => user.id === currentUserId);
                if (currentUserData) {
                    applyTopBarPermissions(currentUserData.rank);
                }
            } catch (error) {
