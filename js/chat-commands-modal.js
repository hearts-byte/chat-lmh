        ...
        openActionModal(modalInfo[key].title, modalInfo[key].content, {
          ...
          onReady: (m, close) => {
            ...
            confirmBtn.onclick = async () => {
              ...
              // الكتم في Firestore، فتظهر الرسالة مرتين في الدردشة العامة لكل
              // من يفتحها (المرسل، الشخص المكتوم، وأي شخص بعد تحديث الصفحة)،
              // لأنهما فعلياً مستندان حقيقيان مختلفان وليس مجرد تكرار عرض.
              if (confirmBtn.disabled) return;
              confirmBtn.disabled = true;

              const muteDuration = durationSelect.value;
              // ✨ نص مدة الكتم كما يظهر للمستخدم في القائمة (مثلاً "5 دقائق" أو "دائم")
              const muteDurationLabel = durationSelect.options[durationSelect.selectedIndex].text;
              ...
              try {
                ...
                // ✨ نص إشعار الشخص المكتوم أصبح: "تم الكتم لمدة (الوقت المحدد)"
                await addNotification(`تم الكتم لمدة ${muteDurationLabel}`, SYSTEM_USER, userIdToMute);
