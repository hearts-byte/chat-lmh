...
import { updateUserData, addLike, removeLike, SYSTEM_USER, addNotification } from './chat-firestore.js';
                ...
                if (hasLiked) {
                    ...
                    if (didLike) {
                        ...
                        if (likerData) {
                            ...
                            addNotification('أعجب بملفك الشخصي', likerSender, likedUserId);
                        }
...
export function showNotificationsModal() {
    ...
    // ✨ لمنع ظهور الإشعار الجديد وحده للحظة ثم ظهور بقية الإشعارات بعده: نتجاهل أي
    // استجابة أولية تصل من الكاش المحلي فقط، وننتظر أول استجابة مؤكدة من الخادم
    // تحتوي على القائمة الكاملة، ثم نعرضها دفعة واحدة.
    let hasReceivedServerSnapshot = false;

    window.notificationsModalUnsubscribe = onSnapshot(notificationsQuery, (snapshot) => {
        if (!hasReceivedServerSnapshot && snapshot.metadata.fromCache) {
            return;
        }
        hasReceivedServerSnapshot = true;

        // ✨ إخفاء مؤشر التحميل فور وصول أول استجابة مؤكدة (سواء كانت فارغة أو لا)
